...
import { getPayHereRedirectUrl, verifyPayHereSignature } from '../services/payhereService.js';
...
export const initiatePayment = async (req, res, next) => {
  try {
    ...
    if (order.customerId !== req.user.id) return res.status(403).json({ message: 'Not your order' });
    if (order.status !== 'pending_payment') return res.status(400).json({ message: 'Order not payable' });
